...
class Structure_U_Net(nn.Module):
    def __init__(self,img_ch=3,output_ch=1):
        super(Structure_U_Net,self).__init__()
...
class Analyzer_U_Net(nn.Module):
    def __init__(self,img_ch=1,output_ch=1):
        super(Analyzer_U_Net,self).__init__()
        self.Maxpool = nn.MaxPool2d(kernel_size=2,stride=2)

        self.Conv1 = conv_(ch_in=img_ch,ch_out=16)
        self.Conv2 = conv_(ch_in=16,ch_out=32)
        self.Conv3 = conv_(ch_in=32,ch_out=64)
        self.Conv4 = conv_(ch_in=64,ch_out=64)

        self.Up4 = up_conv(ch_in=64,ch_out=64)
        self.Up_conv4 = conv_(ch_in=128, ch_out=64)
        
        self.Up3 = up_conv(ch_in=64,ch_out=32)
        self.Up_conv3 = conv_(ch_in=64, ch_out=32)
        
        self.Up2 = up_conv(ch_in=32,ch_out=16)
        self.Up_conv2 = conv_(ch_in=32, ch_out=16)
